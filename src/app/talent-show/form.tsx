...
export function TalentShowForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        router.push("/talent-show/thank-you");
        return;
      }
      setError(
        data.message ||
  ...
  return (
    <form className="mt-12 space-y-10" onSubmit={handleSubmit}>
      ...
      {error && (
        <div
          role="alert"
          className="rounded-md border-2 border-[var(--accent-orange)] bg-black/40 px-4 py-3 text-sm text-[var(--accent-orange)]"
        >
          {error}
        </div>
      )}
      ...
      <div className="flex justify-center pt-2">
        <button
          type="submit"
          disabled={submitting}
          className="inline-flex items-center rounded-full border-2 border-[var(--foreground)] bg-[var(--accent-pink)] px-8 py-3 font-[family-name:var(--font-display)] text-base uppercase tracking-[0.15em] text-black hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
        >
